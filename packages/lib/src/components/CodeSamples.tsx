...
export function CodeSamples({ method, path, parameters, requestBody, security, id }: CodeSamplesProps) {
  const { document, deref, showCodeSamples } = useDocumentContext();
  ...
  // The dropdown's "Agent Prompt" entry (httpsnippet's `agent`/`prompt`
  // target/client, the default selection) is overridden here rather than
  // generated by httpsnippet itself: httpsnippet's own agent prompt is a
  // terse request-only summary, whereas the constructed endpoint Markdown
  // (also used by the document's "Copy for LLM" export) is a much richer,
  // more useful prompt body. Every other target/client still goes through
  // httpsnippet's `generateSnippet` unchanged.
  const snippet = useMemo(() => {
    if (targetKey === DEFAULT_CODE_SAMPLE_TARGET && clientKey === DEFAULT_CODE_SAMPLE_CLIENT) {
      return openApiEndpointToMarkdown(doc, method, path, deref);
    }
    return generateSnippet(harRequest, targetKey, clientKey);
  }, [harRequest, targetKey, clientKey, doc, method, path, deref]);
